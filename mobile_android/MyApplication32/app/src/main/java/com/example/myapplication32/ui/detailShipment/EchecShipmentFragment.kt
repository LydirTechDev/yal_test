...
class EchecShipmentFragment : DialogFragment() {
    ...
    override fun onCreateView(
        ...
        val rootView: View = inflater.inflate(R.layout.fragment_echec_shipment, container, false)
        apiClient = ApiClient()
        sessionManager = SessionManager(rootView.context)

        var radioGroup = rootView.findViewById<RadioGroup>(R.id.radioEcheque)

        rootView.findViewById<Button>(R.id.id_btn_validate).setOnClickListener {
            ...
            this.apiClient.getApiService(rootView.context).setShipmentEchc(
                setShipmentAlertEchecRequest(detailShipmentViewModel.shipment.value.toString(),radioButton.text.toString())
        ...
        rootView.findViewById<Button>(R.id.idi_btn_close).setOnClickListener {
            dismiss()
        }
        return rootView
    }
