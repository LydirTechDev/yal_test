...
export class ListShipmentsComponent implements OnInit {
  ...
  getPaginationShipments() {
    this.isLoading=true;
      this.serviceClientService.getPaginateShipments().subscribe(
        (response)=>{
  ...
  searchTermUpdate(searchShipmentTerm: string): void {
    this.isLoading = true;
    setTimeout(() => {
      this.serviceClientService.searchShipments(searchShipmentTerm).subscribe(
        (response) => {
          this._shipmentsResponse(response);
        },
        (error) => {
  ...
  funcPaginate(link: string, page?: number): void {
    this.isLoading = true;
    setTimeout(() => {
      this.serviceClientService.funcPaginate(link, page,this.searchShipmentTerm).subscribe(
        (response) => {
          this._shipmentsResponse(response);
        },
        (error) => {
  ...
  exportShipments(termToSearch:string) {
    ...
    this.serviceClientService.ExportToExcel(termToSearch).subscribe(
      (response)=>{
  ...
  print(id: number) {
    return this.serviceClientService.printBordereau(id).subscribe({
      next: (response) => {
