...
@Component({
  selector: 'app-liste-recolte',
  templateUrl: './liste-recolte.component.html',
  styleUrls: ['./liste-recolte.component.scss']
})
export class ListeRecolteComponent implements OnInit {
  recolteData: any[];
  /**
   * include page
...
getPaginatedRecolte() {
  this.isLoading = true;
  setTimeout(() => {
    this.serviceClientService.getPaginateRecolteTracabilite().subscribe(
      (response) => {
 ...
 */
  ...
  this.isLoading = true;
  setTimeout(() => {
  this.serviceClientService.funcPaginateRecolteOfUser(link, page).subscribe(
    (response) => {
...
searchTermUpdateRecolte(searchRecolte: string): void {
  this.isLoading = true;
  setTimeout(() => {
  this.serviceClientService.searchRecolteOfUser(searchRecolte).subscribe(
    (response) => {
      this._recoltesResponse(response);
    },
    (error) => {
...
print(id: number, typeRtc: string) {
  return this.serviceClientService.printRecolte(id, typeRtc).subscribe({
    next: (response) => {
