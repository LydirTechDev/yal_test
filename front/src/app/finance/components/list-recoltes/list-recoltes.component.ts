...
getPaginatedRecolte() {
  ...
  setTimeout(() => {
    this.financeService.getPaginateAllRecolte().subscribe(
      (response) => {
      console.log("🚀 ~ file: list-recoltes.component.ts ~ line 80 ~ ListRecoltesComponent ~ setTimeout ~ response", response)
...
print(id: number) {
  return this.financeService.printRecolte(id).subscribe({
    next: (response) => {
...
openFile(data: any, type: string) {
  ...
  calculateDiff(dateSent) {
    ...
    const res = Math.floor((Date.UTC(currentDate.getFullYear(), currentDate.getMonth(), currentDate.getDate()) - Date.UTC(dateSent.getFullYear(), dateSent.getMonth(), dateSent.getDate())) / (1000 * 60 * 60 * 24));
 console.log("🚀 ~ file: list-recoltes.component.ts ~ line 186 ~ ListRecoltesComponent ~ calculateDiff ~ res", res)
      return res;
  }
}
