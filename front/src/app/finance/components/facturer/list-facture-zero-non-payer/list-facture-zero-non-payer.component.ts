...
@Component({
  selector: 'app-list-facture-zero-non-payer',
  templateUrl: './list-facture-zero-non-payer.component.html',
  styleUrls: ['./list-facture-zero-non-payer.component.scss']
})
export class ListFactureZeroNonPayerComponent implements OnInit {
    ...
    paiementForm=this.formBuilder.group({
      ...
      searchTermUpdate(searchFactureTerm: string): void {
        this.isLoading = true;
        setTimeout(() => {
          this.factureService.searchFacture(searchFactureTerm,'non','ecommerceZero').subscribe(
            (response) => {
      ...
      getAllpaginateFacture() {
        this.isLoading = true;
        this.factureService.getAllPaginateFacture('non','ecommerceZero').subscribe(
          (response) => {
            this._facturesResponse(response);
      ...
      funcPaginate(link: string, params?: number) {
        this.isLoading = true;
        setTimeout(() => {
          this.factureService.funcPaginate(link, params, this.searchFactureTerm,'non','ecommerceZero').subscribe(
            (response) => {
              this._facturesResponse(response);
            },
            (error) => {
      ...
      payer() {
        this.factureService.payerFacture(this.factureId,this.paiementForm.value).subscribe(
          (response)=>{
            ...
            this.router.navigateByUrl(`finance/list-facture-zero-payer`);
